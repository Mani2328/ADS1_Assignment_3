...
def missing_data(df):
    """
    Calculates the missing data in each columns of the dataframe
    
    Parameter
    arg1 : dataframe
    
    Return:
    % of missing data: float
    """ 
...
#Check for missing data with missing_data function
missing_data(df)
...
plt.ylim(6,12)
plt.title('CO2 Emission - Germany')
plt.xlabel('Year')
plt.ylabel('CO2 emissions (metric tons per capita)')
plt.legend()
plt.show()
...
#Predict CO2 emission level - 2nd attempt with fitting parameters
df_year['C02_Germany_fit2'] = exp_decay(df_year['Year'], *popt)
...
plt.legend()
plt.title('CO2 Emission - Germany', fontsize=15)
plt.xlabel('Year')
plt.ylabel('CO2 emissions (metric tons per capita)')
plt.show()
...
plt.plot(years, pop_decay, label ='fit')
plt.fill_between(years, low, high, alpha=0.5, color='y')
plt.title('CO2 Emission - Singapore', fontsize=15)
plt.xlabel('Year')
plt.ylabel('CO2 emissions (metric tons per capita)')
plt.legend()
plt.show()
...
plt.fill_between(years, low, high, alpha=0.5, color='y')
plt.title("CO2 Emission - South Korea", fontsize=15)
plt.xlabel('Year')
plt.ylabel('CO2 emissions (metric tons per capita)')
plt.legend()
plt.show()
